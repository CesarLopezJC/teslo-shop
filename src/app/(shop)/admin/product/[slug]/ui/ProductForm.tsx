...
import { ProductImage as ProductImageComp } from "@/components";
...
export const ProductForm = ({ product, categories }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        const { ok, product: product1 } = await createUpdateProduct(formData);

        if (!ok) {
            alert(`Could not ${product.id ? 'update' : 'create'} the product`);
            return;
        }

        router.replace(`/admin/product/${product1?.slug}`);

        console.log({ ok });
    }

    const onDeleteProductImage = async (id: number) => {
        const deleteResp = await deleteProductImage(id);

        if (!deleteResp.ok)
            alert(deleteResp.message);
    }
